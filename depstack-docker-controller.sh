...
function init_data(){
    ...
    docker exec keystone su -s /bin/bash keystone -c "keystone-manage db_sync"
    #初始化Fernet密钥存储库
    docker exec keystone keystone-manage fernet_setup --keystone-user keystone --keystone-group keystone
    docker exec keystone keystone-manage credential_setup --keystone-user keystone --keystone-group keystone
    #引导身份服务，密码为keystone
    docker exec keystone keystone-manage bootstrap --bootstrap-password keystone \
    ...
    docker exec keystone  bash -c 'source /root/admin-openrc.sh && openstack project create --domain default --description "Demo Project" demo'
    #创建demo用户
    docker exec keystone  bash -c 'source /root/admin-openrc.sh && openstack user create --domain default --password demo demo'
    #创建user角色
    docker exec keystone  bash -c 'source /root/admin-openrc.sh && openstack role create user'
    ...
    docker exec keystone  bash -c 'source /root/admin-openrc.sh && openstack user create --domain default --password glance glance'
    #将admin角色添加到glance用户和service项目上
    docker exec keystone  bash -c 'source /root/admin-openrc.sh && openstack role add --project service --user glance admin'
    #创建glance项目
    docker exec keystone  bash -c 'source /root/admin-openrc.sh && openstack service create --name glance --description "OpenStack Image" image'
    #创建访问glance的endpoint端点
    docker exec keystone  bash -c 'source /root/admin-openrc.sh && openstack endpoint create --region RegionOne image public http://controller:9292  && \
    ...
    #创建nova用户
    docker exec keystone  bash -c 'source /root/admin-openrc.sh && openstack user create --domain default --password nova nova'
    #添加admin角色给nova用户
    docker exec keystone  bash -c 'source /root/admin-openrc.sh && openstack role add --project service --user nova admin'
    #创建nova项目
    docker exec keystone  bash -c 'source /root/admin-openrc.sh && openstack service create --name nova --description "OpenStack Compute" compute'
    #创建访问nova的endpoint端点
    docker exec keystone  bash -c 'source /root/admin-openrc.sh && openstack endpoint create --region RegionOne compute public http://controller:8774/v2.1 && \
    openstack endpoint create --region RegionOne compute internal http://controller:8774/v2.1 && \
    openstack endpoint create --region RegionOne compute admin http://controller:8774/v2.1'
    #创建placement用户
    docker exec keystone  bash -c 'source /root/admin-openrc.sh && openstack user create --domain default --password placement placement'
    #添加placement user到service project，并且赋予admin角色
    docker exec keystone  bash -c 'source /root/admin-openrc.sh && openstack role add --project service --user placement admin'
    #创建placement服务的项目
    docker exec keystone  bash -c 'source /root/admin-openrc.sh && openstack service create --name placement --description "Placement API" placement'
    #创建访问placement的endpoint端点
    docker exec keystone  bash -c 'source /root/admin-openrc.sh && openstack endpoint create --region RegionOne placement public http://controller:8778 && \
    ...
    #创建neutron用户
    docker exec keystone  bash -c 'source /root/admin-openrc.sh && openstack user create --domain default --password neutron neutron'
    #添加admin角色给neutron用户
    docker exec keystone  bash -c 'source /root/admin-openrc.sh && openstack role add --project service --user neutron admin'
    #创建neutron服务的项目
    docker exec keystone  bash -c 'source /root/admin-openrc.sh && openstack service create --name neutron --description "OpenStack Networking" network'
    #创建访问网络服务的endpoint端点
    docker exec keystone  bash -c 'source /root/admin-openrc.sh && openstack endpoint create --region RegionOne network public http://controller:9696 && \
